...
for (var i = 0; i < buttons.length; i++) { // лучше не использовать объявление переменной  с ключевым словом var
...
catch (err) { // ошибка в const res = await askBackend(); здесь не cловим
...
var arr = [1, 1]; // объявляем  массив [1, 1]
arr[2] = 1; // добавляем в маcсив по индексу 2 значение три - 1
console.log(arr[6] = undefined); // добавляем в массив по индексу 6 значение три undefined, создаются ячейки массива с индекса 2 до 6
...
// 2. -Imagine that you are designing a new huge website with React. How would you implement localization?
// Give simple snippets for a button and a list element (of your choice). A list contains languages (ENG, RUS, CH, JAP),
// button confirms the selection and changes its language. Language is saved after reload, default - ENG.
...
// 3. - Assume that you are working on a relatively big project.
// It is React SPA with a lot of dialogs. You were asked to add Google Analytics on pages and track different actions. What questions would you ask? 
